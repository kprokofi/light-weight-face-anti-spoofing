...
datasets = dict(LCCFASD_root='/home/prokofiev/pytorch/LCC_FASDnew', Celeba_root='/home/prokofiev/pytorch/antispoofing/CelebA_Spoof')
...
loss = dict(loss_type='amsoftmax', 
            amsoftmax=dict(m=0.5, s=1, margin_type='cross_entropy', label_smooth=False, smoothing=0.1, ratio=[1,1]),
            soft_triple=dict(cN=2, K=10, s=1, tau=.2, m=0.35))

epochs = dict(start_epoch=0, max_epoch=71)

model= dict(model_type='Mobilenet3', model_size = 'large', use_amsoftmax=True, pretrained=True, embeding_dim=512)
...
dropout = dict(prob_dropout=0.1, classifier=0.5, type='bernoulli', mu=0.5, sigma=0.3)
